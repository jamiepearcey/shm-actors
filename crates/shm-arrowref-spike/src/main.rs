...
fn main() -> Result<(), shm_arrowref_spike::SpikeError> {
    ...
    println!("---------------------------------------------");
    println!("input payload retained once : {:>8} bytes", report.input_payload_bytes);
    println!("control message on the queue: {:>8} bytes (ChunkDesc)", report.control_msg_bytes);
    println!("full task-queue slot        : {:>8} bytes", report.queue_slot_bytes);
    println!("payload : control ratio     : {:>8}x", report.payload_to_control_ratio);
    println!(
        "retained output ref         : dataset={:?} version={}",
        report.output.dataset, report.output.version
    );
    println!("output rows                 : {:>8}", report.output_rows);
    println!("input read zero-copy        : {}", report.input_read_zero_copy);
    println!("output read zero-copy       : {}", report.output_read_zero_copy);
