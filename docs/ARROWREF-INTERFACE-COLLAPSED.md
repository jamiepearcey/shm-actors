...
| **6. Topics pub/sub** | `shm-ring` SPMC broadcast + cursors | ✅ **same-host** ring; ✅ resumable seek (`Subscriber::from_seq` / `Node::subscribe_from`) | ❌ **SSE + WebSocket** browser transport (no HTTP in substrate); node-plane `seq` sequencer, retention policy, durability, tenant prefixing; publish is **single-producer** (serialise multi-caller publish) |
